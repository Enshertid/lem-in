...
/*   Updated: 2020/02/05 16:13:29 by user             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "map.h"

static void			put_circle(t_wnd *wnd, t_img *img, int x, int y)
{
...
{
	put_circle(&data->wnd, &data->wnd.start_circle_img,
				data->graph.rooms[0].coord.x, data->graph.rooms[0].coord.y);
	put_circle(&data->wnd, &data->wnd.end_circle_img,
				data->graph.rooms[1].coord.x, data->graph.rooms[1].coord.y);
