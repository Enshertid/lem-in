...
/*   Updated: 2020/02/14 03:19:07 by dbendu           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		rotate_x(t_data *data)
{
	...
	while (iter < rooms_amount)
	{
		...
		if (z > 0)
			...
			z -= data->camera.z;
		data->graph.rooms[iter].coord.y = y * data->appearance.angles.cosx +
											z * data->appearance.angles.sinx;
		data->graph.rooms[iter].coord.z = -y * data->appearance.angles.sinx +
											z * data->appearance.angles.cosx;
		++iter;
	}
}
...
static void		rotate_y(t_data *data)
{
	...
	while (iter < rooms_amount)
	{
		...
		data->graph.rooms[iter].coord.x = x * data->appearance.angles.cosy +
											z * data->appearance.angles.siny;
		data->graph.rooms[iter].coord.z = -x * data->appearance.angles.siny +
											z * data->appearance.angles.cosy;
		++iter;
	}
}
...
static void		rotate_z(t_data *data)
{
	...
	while (iter < rooms_amount)
	{
		...
		data->graph.rooms[iter].coord.x = x * data->appearance.angles.cosz -
											y * data->appearance.angles.sinz;
		data->graph.rooms[iter].coord.y = x * data->appearance.angles.sinz +
											y * data->appearance.angles.cosz;
